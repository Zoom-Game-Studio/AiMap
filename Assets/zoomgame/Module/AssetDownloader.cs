...
namespace Waku.Module
{
    public class AssetDownloader : MonoBehaviour, IResDownload<AssetList>
    {
        ...
        public void DownloadAssetByGps(Vector3 gps)
        {
            var info = FindAssetByGps(gps);
            if (info != null)
            {
        ...
        public AssetInfoItem FindAssetByGps(Vector2 gps)
        {
        ...
        void AddToDownloadList(AssetInfoItem serverItemInfo, Action callback = null)
        {
            //需要下载的item
        ...
        void StartDownLoad(Action callback = null)
        {
            ...
            loader.DownLoad(file.link, localFullPath, zipName, () =>
            {
                ...
                if (localItem == null)
                {
                    localItem = info.Clone();
                    info.package.files.Clear();
                    ClientList.Add(localItem);
                }

                //删除本地存在的file记录，记录新下载的file
                var localFiles = localItem.package.files;
                var localFile = localFiles.Find(f => f.id == file.id);
                if (localFile != null)
                {
