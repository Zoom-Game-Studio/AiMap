...
namespace ARLocation
{
    ...
    public class PlaceAtLocation : MonoBehaviour
    {
        ...
        static Location thisLocation = null;

        private void Awake()
        {
            AppFacade.GetInstance().addEvent(ModuleEventType.UPDATE_POI_INFO, HandlewUpdatePOIInfo);
        }

        private void HandlewUpdatePOIInfo(EventObject ev)
        {
            Debug.Log("HandleUPdatePOiInfo...");
            Location location = ev.param as Location;
            if (location == null) return;

            var distance = Location.HorizontalDistance(location, this.LocationOptions.LocationInput.Location);
            ...
            if (distanceTextDetailObj != null)
            {
                ...
                {
                    distanceTextDetail.text = distance.ToString("0") + "m";
                }
            }
        }
        ...
        {
            Debug.Log("CreatePlacedInstance:"+go.name);
        ...
        {
            Debug.Log("AddPlaceAtComponent:" + go.name);
            ...
            GameObject nameTextDetailObj = UnityUtilties.FindChild(go.transform, "NameTextDetail");
            if (nameTextDetailObj != null)
            {
                Text nameTextDetail = nameTextDetailObj.GetComponent<Text>();
                if (nameTextDetail != null)
                {
                    nameTextDetail.text = location.Label;
                }
            }
            go.AddComponent<LookAtCamera>();
            GameObject distanceTextDetailObj = UnityUtilties.FindChild(go.transform, "DistanceTextDetail");
            if (distanceTextDetailObj != null)
            {
                Text distanceTextDetail = distanceTextDetailObj.GetComponent<Text>();
                if (distanceTextDetail != null)
                {
                    distanceTextDetail.text = "0m";
                }
            }
